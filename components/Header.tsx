      ...
      <header className="fixed top-0 left-0 right-0 bg-zinc-950 border-b border-zinc-800 z-50">
        <div className="max-w-7xl mx-auto px-6 py-5">
          <div className="flex items-center justify-between">
            ...
            {/* Full Navigation Menu - Perfectly centered on desktop */}
            <nav className="hidden md:flex items-center gap-8 text-sm font-medium absolute left-1/2 -translate-x-1/2">
              <Link href="/metamask" className={isActive('/metamask') ? 'text-blue-400' : 'hover:text-zinc-300 transition-colors'}>MetaMask</Link>
              <Link href="/grokoracle" className={isActive('/grokoracle') ? 'text-blue-400' : 'hover:text-zinc-300 transition-colors'}>GROKoracle</Link>
              <Link href="/chat" className={isActive('/chat') ? 'text-blue-400' : 'hover:text-zinc-300 transition-colors'}>Chat</Link>
              <Link href="/dns" className={isActive('/dns') ? 'text-blue-400' : 'hover:text-zinc-300 transition-colors'}>DNS</Link>
              <Link href="/bdagscan" className={isActive('/bdagscan') ? 'text-blue-400' : 'hover:text-zinc-300 transition-colors'}>BdagScan</Link>
            </nav>
